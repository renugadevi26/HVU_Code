...
layers.tf = tf
print('model prepration')

from tensorflow.keras.preprocessing.image import ImageDataGenerator

# Define functions for model building
def create_resnet50_encoder(input_shape):
    base_model = ResNet50(weights=None, include_top=False, input_shape=input_shape)
    inp = Input(shape=input_shape)
    output_layer = base_model(inp)
...
callbacks = [
    ModelCheckpoint("/nfsshare/knr/knrtrain/renu/resnet_vit/brain_tumor_vit_res_weights.h5", verbose=1, save_best_only=True, monitor='val_loss', mode='min', save_weights_only=True),
    ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=1e-7, verbose=1),
    CSVLogger("/nfsshare/knr/knrtrain/renu/resnet_vit/brain_tumor_vit_res_csv.log"),
]
...
unet_model.load_weights("/nfsshare/knr/knrtrain/renu/resnet_vit/brain_tumor_vit_res_weights.h5")
